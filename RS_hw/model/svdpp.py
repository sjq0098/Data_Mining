...
# 构建隐式反馈集 N(u)
user_interactions = raw_df.groupby('u_idx')['i_idx'].apply(list).to_dict()
...
class SVDpp(nn.Module):
    def __init__(self, n_users, n_items, emb_dim=32, reg=1e-4):
        super().__init__()
        # 全局均值
        ...
        # 隐向量 p_u, q_i
        self.pu = nn.Embedding(n_users, emb_dim)
        self.qi = nn.Embedding(n_items, emb_dim)
        # 隐式反馈项 y_j
        self.yj = nn.Embedding(n_items, emb_dim)
        # 正则化强度
        self.reg = reg
        # 初始化
        nn.init.constant_(self.bu.weight, 0)
        nn.init.constant_(self.bi.weight, 0)
        nn.init.normal_(self.pu.weight, std=0.01)
        nn.init.normal_(self.qi.weight, std=0.01)
        nn.init.normal_(self.yj.weight, std=0.01)

    def forward(self, u, i, user_interactions):
        # u, i: batch of indices
        pu = self.pu(u)        # [B, D]
        qi = self.qi(i)        # [B, D]
        bu = self.bu(u).squeeze()  # [B]
        bi = self.bi(i).squeeze()  # [B]
        # 构建隐式反馈聚合 e_u = |N(u)|^{-1/2} sum_{j in N(u)} y_j
        emb_dim = pu.size(1)
        e_u = []
        for uid in u.cpu().tolist():
            idxs = user_interactions.get(uid, [])
            if len(idxs) > 0:
                y_sum = self.yj(torch.LongTensor(idxs).to(u.device)).sum(0)
                e_u.append(y_sum / np.sqrt(len(idxs)))
            else:
                e_u.append(torch.zeros(emb_dim, device=u.device))
        e_u = torch.stack(e_u, dim=0)  # [B, D]
        # 预测评分
        r_hat = self.mu + bu + bi + (qi * (pu + e_u)).sum(dim=1)
        return r_hat

    def regularization(self):
        # L2 正则
        return self.reg * (
            self.bu.weight.norm()**2 + self.bi.weight.norm()**2 +
            self.pu.weight.norm()**2 + self.qi.weight.norm()**2 +
            self.yj.weight.norm()**2
        )

# 4. 训练 & 验证

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
model = SVDpp(len(user_list), len(item_list), emb_dim=32, reg=1e-4).to(device)
# 预计算全局均值
model.mu.data.fill_(train_df['score'].mean())
...
for epoch in range(1, 21):
    ...
    for u, i, r in train_loader:
        ...
        pred = model(u, i, user_interactions)
    ...
    with torch.no_grad():
        for u, i, r in val_loader:
            ...
            p = model(u, i, user_interactions).cpu().tolist()
    ...
    if rmse < best_rmse:
        ...
        torch.save(model.state_dict(), 'best_svdpp.pth')
    else:
        trials += 1
        if trials >= patience:
...
# 加载最佳模型并预测
state = torch.load('best_svdpp.pth', map_location=device)
...
for _, row in test_df.iterrows():
    ...
    if u >= 0 and i >= 0:
        ...
        score = model(u_t, i_t, user_interactions).item()
    else:
        score = mean_score
    preds.append(score)

test_df['score'] = preds
test_df.to_csv('test_svdpp_preds.csv', index=False)

with open('test_svdpp_preds.txt', 'w', encoding='utf-8') as f:
